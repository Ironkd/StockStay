...
2. Copy env examples and set local values:
   ...
   PORT=3000
   DATABASE_URL=postgresql://stockstay:stockstay@localhost:5433/stockstay
...
3. Apply schema and run the API:
   ...
   cd server
   npx prisma generate
   # Fresh local DB: migration history has no baseline create-all migration.
   # Use db push for a new Docker volume; use migrate deploy against staging/prod that already have history.
   npx prisma db push
   npm run dev
   ```
...
9. Set `APP_URL` = the Vercel staging URL.  
10. Set `CORS_ORIGIN` = the same Vercel staging URL.  
11. Redeploy the Railway staging service so the new variables apply.
